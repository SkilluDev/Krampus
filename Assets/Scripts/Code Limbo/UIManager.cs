...
public class UIManager : MonoBehaviour {
    ...
    private void Update() {
        ...
        if (m_inTutorial) {
            if (WinCondition.Instance.inputSubscribe.AdvanceInput) { //Input.GetKeyUp(KeyCode.G) //Old input system
...
//        Debug.Log("W");
    ...
    public void ActivateGameOverScreen(WinCondition.LostGameCase lostGameCase) {
        ...
        Time.timeScale = 0f;
        m_gameOverText.transform.GetChild(0).GetComponent<TextMeshProUGUI>().text = "Time elapsed: " + (int)WinCondition.Instance.totalTime + "\nNaughty children eaten: " + Interaction.Instance.badChildrenEatCount + "\nGood children eaten: " + Interaction.Instance.goodChildrenEatCount;
        /*switch (lostGameCase)
        {
