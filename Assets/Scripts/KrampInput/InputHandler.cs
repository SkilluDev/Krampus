...
public class InputHandler : MonoBehaviour
{
	...
	private void Update() {
		...
		if (Game.Balling || Game.MainGameInfo.CurrentState == RoundInfo.State.Paused) {
			if (InputSubscribe.Raw.UI.Pause.triggered) SwitchPause();
		}

		if (Game.roundInfo.Lost || Game.MainGameInfo.CurrentState == RoundInfo.State.Paused) {//if the game is over, or paused, you can

			if ((InputSubscribe.Raw.UI.Pause.triggered || InputSubscribe.Raw.UI.Advance.triggered )&& Game.PogMan.CanGoToNextLevel) {
				//go to next level with space
		...
		if (Game.roundInfo.Won) {//if the game is won, you can
			/* if (InputSubscribe.Raw.UI.MenuReturn.triggered) { //go back to menu with M
				Game.PogMan.GoBackToMenu();
			} */
    ...
    public static void SwitchPause() {
        if (!Game.roundInfo.Ended) {
            if (Game.MainGameInfo.CurrentState == RoundInfo.State.Paused) {
                Game.MainGameInfo.SetState(RoundInfo.State.Game);
                Game.MainGameInfo.UI.SwitchPauseMenu(false);
            } else {
                Game.MainGameInfo.SetState(RoundInfo.State.Paused);
                Game.MainGameInfo.UI.SwitchPauseMenu(true);
            }
            Time.timeScale = Game.MainGameInfo.CurrentState != RoundInfo.State.Paused ? 1 : 0;
        }
    }
}
