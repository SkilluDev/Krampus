...
public class AimStick : MonoBehaviour
{
    ...
    void Update()
    {
	    rightStick = InputSubscribe.Input.Player.AimPad.ReadValue<Vector2>();
	    if (rightStick.magnitude < 0.1f) return; //suppress input jitter during stick inactivity
	    mousePosition = InputSubscribe.Input.Player.Aim.ReadValue<Vector2>();//capture mouse position to add to stick movement
